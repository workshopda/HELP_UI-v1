			...
			>
				<div class="">
					...
					{:else}
						...
						>
							<div 
								class="flex-1 flex flex-col relative w-3/4 min-h-[110px] shadow-lg rounded-lg 
									border border-gray-50 dark:border-gray-850 hover:border-gray-100 focus-within:border-gray-100 
									hover:dark:border-gray-800 focus-within:dark:border-gray-800 transition px-1 bg-white/90 
									dark:bg-gray-400/5 dark:text-gray-100 mx-auto my-10"
								...
								<div class="px-2.5">
									{#if $settings?.richTextInput ?? true}
										<div
											class="scrollbar-hidden rtl:text-right ltr:text-left bg-transparent dark:text-gray-100 outline-hidden w-full pt-3 pb-2 px-2 resize-none h-fit max-h-[350px] overflow-auto "
								...
								<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
									<div class="ml-1 self-end flex items-center flex-1 max-w-[80%]">
										...
										>
											...
											class="relative flex items-center justify-center bg-transparent hover:bg-gray-100 text-gray-800 dark:text-white dark:hover:bg-gray-800 transition border border-gray-300 dark:border-gray-600 w-8 h-8 p-[0.4375rem] rounded-md translate-y-2"
											onclick="this.classList.toggle('active')"
											>
											<!-- Plus Icon -->
											<svg
												class="icon-plus absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-4 transition-all duration-300 ease-in-out"
												xmlns="http://www.w3.org/2000/svg"
											...
											<!-- Close Icon -->
											<svg
												class="icon-close absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-4 transition-all duration-300 ease-in-out"
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 -960 960 960"
												fill="currentColor"
											>
												...
												<div class="flex gap-3 mt-4">
													{#if showWebSearchButton}
														<Tooltip content={$i18n.t('Search the internet')} placement="top">
															<button
																...
																type="button"
																class="sticky top-0 p-1.5 flex items-center gap-1.5 text-sm rounded-md transition-colors duration-300 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:outline-none translate-y-0 shrink-0
																	{webSearchEnabled ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'hover:bg-gray-100 dark:hover:bg-gray-800'}"
															>
																<GlobeAlt class="size-4" strokeWidth="1.75" />
																<span class="hidden @xl:block">{$i18n.t('Search')}</span>
															</button>
														</Tooltip>
													{/if}
													...
													<Tooltip content={$i18n.t('Generate an image')} placement="top">
														<button
															...
															type="button"
															class="p-2 flex items-center gap-2 text-sm rounded-md transition-colors duration-300 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:outline-none"
														>
															<Photo class="size-4" strokeWidth="1.75" />
															<span class="hidden @xl:block">{$i18n.t('Image')}</span>
														</button>
													</Tooltip>
													{/if}

													{#if false && showCodeInterpreterButton}
														<Tooltip content={$i18n.t('Execute code for analysis')} placement="top">
															<button
																on:click|preventDefault={() => (codeInterpreterEnabled = !codeInterpreterEnabled)}
																type="button"
																class="p-2 flex items-center gap-2 text-sm rounded-none transition-colors duration-300 hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:outline-none"
															>
																<CommandLine class="size-4" strokeWidth="1.75" />
																<span class="hidden @xl:block">{$i18n.t('Code')}</span>
															</button>
														</Tooltip>
													{/if}
												</div>
										...
										{#if (!history?.currentId || history.messages[history.currentId]?.done == true) && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true))}
											...
											<Tooltip content={$i18n.t('Dictate')}>
												<button
													...
													class="text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 transition rounded-md border border-gray-200 dark:border-gray-700 p-1.5 mr-0.5 self-center"													type="button"
													on:click={async () => {
														try {
										...
										{#if (taskIds && taskIds.length > 0) || (history.currentId && history.messages[history.currentId]?.done != true)}
											<div class="flex items-center">
												<Tooltip content={$i18n.t('Stop')}>
													<button
													class="bg-gray-700 text-white hover:bg-gray-600 border border-gray-600 rounded-md transition p-1.5"
													on:click={() => {
														stopResponse();
													}}
													>
										...
										{:else if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<Tooltip content={$i18n.t('Voice mode')}>
													<button
														class="bg-black text-white hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-md border border-gray-300 dark:border-gray-600 w-8 h-8 p-1.5 self-center"
														type="button"
														on:click={async () => {
										...
										{:else}
											...
											<Tooltip content={$i18n.t('Send Message')}>
												...
												class="{!(prompt === '' && files.length === 0)
													? 'bg-gray-700 text-white hover:bg-gray-600 border border-gray-600 rounded-md'
													: 'text-gray-400 bg-gray-200 border border-gray-300 cursor-not-allowed dark:bg-gray-800 dark:border-gray-700'} transition p-1.5 self-center"
												type="submit"
